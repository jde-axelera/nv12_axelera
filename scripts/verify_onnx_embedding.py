...
binary (direct resize to 224x224, ImageNet normalise) and prints the first
6 embedding values.  Compare with the AIPU output to gauge quantisation error.

Usage:
    # print ONNX embedding
    python scripts/verify_onnx_embedding.py \\
        --onnx models/resnet18_embedding.onnx \\
        --image input_images/dog_bike_768x576.rgba \\
        --size 768x576

    # compare with saved AIPU embedding
    python scripts/verify_onnx_embedding.py \\
        --onnx models/resnet18_embedding.onnx \\
        --image input_images/dog_bike_768x576.rgba \\
        --size 768x576 \\
...
def preprocess(rgba, target=224):
    """Matches C++ rgba_to_tensor_imagenet: resize then ImageNet normalise."""
    ...
    rgb  = (rgb - mean) / std                                # HWC normalised
