// YOLOv5s object detection with RGBA input and DMA-BUF passthrough.
// Drop-in replacement for main.cpp targeting FPGA sources that output
// raw RGBA frames (4 bytes/pixel) instead of NV12.
...
int main(int argc, char** argv)
{
    ...
    if (model_path.empty()) {
        std::fprintf(stderr,
            ...
            "  image       : JPEG/PNG or raw .rgba file (4 bytes/pixel, RGBA order)\n"
            "  --size=WxH  : raw RGBA dimensions, e.g. --size=1920x1080\n"
    ...
    // FPGA integration note: for zero-copy with an FPGA writing to a fixed
    // physical address, allocate DMA-BUF here first, query the physical address
    // via the axruntime FPGA driver, then configure the FPGA DMA target to that
    // address. The mmap'd ptr below is the CPU-visible write window if needed
    // for an intermediate software path (Option D: memcpy from FPGA mmap).
    ...
    if (!image_path.empty()) {
        ...
        if (ext == "rgba") {
            int sw = rgba_w > 0 ? rgba_w : 1920;
            int sh = rgba_h > 0 ? rgba_h : 1080;
            if (rgba_w == 0) {
                auto us = image_path.rfind('_');
                if (us != std::string::npos)
                    std::sscanf(image_path.c_str() + us + 1, "%dx%d", &sw, &sh);
            }
            const size_t frame_bytes = static_cast<size_t>(sw) * sh * 4;
            ...
            std::printf("[INFO] RGBA input: %dx%d (%zu KB, first frame)\n",
                        sw, sh, frame_bytes / 1024);

            cv::Mat rgba_mat(sh, sw, CV_8UC4, rgba.data());
            cv::cvtColor(rgba_mat, vis_bgr, cv::COLOR_RGBA2BGR);
            orig_w = sw; orig_h = sh;

            rgba_to_tensor(rgba.data(), sw, sh, in_ptr0, in_info[0]);
            ...
            vis_bgr = img.clone();
            std::printf("[INFO] Image %dx%d decoded and converted via RGBA pipeline\n",
                        orig_w, orig_h);

            cv::Mat rgba_img;
            cv::cvtColor(img, rgba_img, cv::COLOR_BGR2RGBA);
            rgba_to_tensor(rgba_img.data, orig_w, orig_h, in_ptr0, in_info[0]);
            out_jpg = out_path.empty()
                ? image_path.substr(0, image_path.rfind('.')) + "_detections.jpg"
                : out_path;
        }
    } else {
        std::printf("[INFO] No image -- using synthetic RGBA 640x640 test pattern\n");
        orig_w = 640; orig_h = 640;
        std::vector<uint8_t> rgba(640 * 640 * 4, 114);
        rgba_to_tensor(rgba.data(), 640, 640, in_ptr0, in_info[0]);
    ...
    // Build RGBA benchmark buffer from vis_bgr (no NV12 interleaving needed)
    cv::Mat rgba_bench_mat;
    {
        cv::Mat src = vis_bgr.empty()
            ? cv::Mat(640, 640, CV_8UC3, cv::Scalar(114, 114, 114))
            : vis_bgr;
        cv::cvtColor(src, rgba_bench_mat, cv::COLOR_BGR2RGBA);
    }
    const int    bench_w   = rgba_bench_mat.cols;
    const int    bench_h   = rgba_bench_mat.rows;
    const size_t rgba_bytes = static_cast<size_t>(bench_w) * bench_h * 4;
    std::vector<uint8_t> rgba_bench(rgba_bench_mat.data,
                                    rgba_bench_mat.data + rgba_bytes);

    // Prime buf[0] before the loop so iteration 0 can launch preprocess into buf[1]
    rgba_to_tensor(rgba_bench.data(), bench_w, bench_h, in_ptrs[0], in_info[0]);
    ...
    for (int i = 0; i < bench; ++i) {
        ...
        const uint8_t*       src_data = rgba_bench.data();
        ...
        auto pre_fut = std::async(std::launch::async,
            [src_data, bench_w, bench_h, nxt_ptr, &tinfo]() -> double {
                ...
                rgba_to_tensor(src_data, bench_w, bench_h, nxt_ptr, tinfo);
